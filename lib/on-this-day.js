...
function fetchAndRespond(app, req, res, titleFunction, extractionFunction, sendResponse = true) {
    ...
    .catch((err) => {
        // Gracefully handle missing day pages (404). Non-Wikipedia wikis
        // won't have day pages like June_27 for every date. Instead of
        // crashing with a 500, return empty data so the feed endpoint
        // continues to work. Day pages can be created incrementally.
        if (err.status === 404 || (err.response && err.response.status === 404)) {
            const emptyResult = { selected: [], events: [], births: [], deaths: [], holidays: [] };
            if (!sendResponse) { return emptyResult; }
